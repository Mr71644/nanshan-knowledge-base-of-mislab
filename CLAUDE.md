...
ComponentName/
  ...
  └── index.module.css    # Compiled CSS (generated by Vite at build time)
```

Styles use CSS Modules: `import style from './index.module.less'`, then `style.className`.
Vite compiles `.module.less` to CSS at build time via the `less` devDependency.

**Exceptions:** `ChatInput` and `TiptapEditor` import `.module.css` directly (no `.less` source, CSS was written directly).

### Multi-theme system

Two themes managed in a single branch via CSS custom properties + JS config + build-time Vite alias:

| Theme | ID | Accent | Brand |
|---|---|---|---|
| 重庆邮电大学 | `cqupt` | `#d4a84c` (铜金) | 知邮南山 / MISLab |
| 广西大学 | `gxu` | `#5e963c` (绿色) | 甘蔗育种中心 / ZhangLab |

**How it works:**
- `src/theme/{id}.css` — CSS custom properties on `:root` (all colors, gradients, shadows)
- `src/theme/{id}.js` — JS config (branding strings, BG.jsx canvas params, Ant Design ConfigProvider tokens)
- `vite.config.js` — Aliases `#theme` and `#theme-css` to the selected theme based on `VITE_THEME` env var
- Components import `#theme` for JS config, `#theme-css` is imported once in `main.jsx`
- All LESS/CSS uses `var(--color-xxx)` instead of hardcoded hex values

**Build commands:**
```bash
pnpm dev          # cqupt theme
pnpm dev:gxu      # gxu theme
pnpm build        # cqupt theme
pnpm build:gxu    # gxu theme
```

**Adding a new theme:** Create `src/theme/newtheme.js` + `src/theme/newtheme.css`, add build script with `cross-env VITE_THEME=newtheme`. No component code changes needed.
